...
function setStringSrc(typeFIle, dataSrc) {
	return typeFIle === ".css"
		? `<link href="${dataSrc}" rel="stylesheet" />`
		...
		[
			...
			inject(src([build], { read: false }), {
				transform: (filepath) => {
					...
					let stringToInsert = filepath.slice(
						filepath.lastIndexOf("/") + 1,
						filepath.lastIndexOf("."),
					);
					if (stringToInsert.lastIndexOf("_") >= 0) {
						stringToInsert = stringToInsert.slice(
							0,
							stringToInsert.lastIndexOf("_"),
						);
					}
					fs.mkdirSync(pathOutput, { recursive: true });
					fs.writeFileSync(
						`${pathOutput}${stringToInsert}.html`,
						setStringSrc(typeFile, fileExtension),
					);
				},
			}),
		],
